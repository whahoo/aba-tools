...
config.column.schema = Joi.object()
  .required()
  .keys({
...
config.recordTypes.descriptive.schema = Joi.object()
  .required()
  .keys({
...
config.recordTypes.detail.schema = Joi.object()
  .required()
  .keys({
    ...
    toRef: Joi.string()
      .regex(/^[1-9A-Za-z]{1}[0-9A-Za-z ]{0,17}$/)
      .required(),
    ...
    transaction: Joi.number()
      .valid(13, 50, 51, 52, 53, 54, 55, 56, 57)
      .optional()
      .default(53),
...
config.recordTypes.total.schema = Joi.object()
  .required()
  .keys({
    //all totals from the other 'detail' records'
    totalNet: Joi.number().min(1).max(99999999).required(),
    totalCredit: Joi.number().min(0).max(99999999).required(),
    totalDebit: Joi.number().min(0).max(99999999).required(),
