...
function testBadInputs(recordType, fieldKey, badInputs, values) {
  ...
  _.each(badInputs, function (badInput) {
    ...
    var errorRegex = new RegExp(
      "^(?=.*\\bValidationError\\b)(?=.*\\b" + fieldKey + "\\b).*$"
    );
    it(
      "should throw ValidationError if " +
        fieldKey +
        " is [ " +
        badInput +
        " ]",
      function () {
        assert.throws(function () {
          getRecord(recordType, badInputs);
        }, errorRegex);
      }
    );
  });
  ...
  _.each(["descriptive", "detail", "total"], function (type) {
    describe("Config for " + type + " Columns", function () {
      it("should get a total of 120 characters", function () {
        var totalCharsInColumns = _.reduce(
          recordTypes[type].columns,
      ...
      it("should validate each column against the column schema", function () {
        _.each(recordTypes[type].columns, function (column) {
          const { error } = config.column.schema.validate(column, config.joi);
...
describe("# utils", function () {
  ...
  describe("descriptive record", function () {
    ...
    describe("bank", function () {
      testBadInputs(
        "descriptive",
        this.title,
        ["not provided", null, undefined, "a", "ab", "abcd", "###", 123, "123"],
        good.descriptive.values
      );
    });
    ...
    describe("description", function () {
      testBadInputs(
        "descriptive",
        this.title,
        ["not provided", null, undefined, "", "very long description", 1232344],
        good.descriptive.values
      );
    });
  ...
  describe("detail record", function () {
    ...
    describe("transaction", function () {
      testBadInputs("detail", this.title, [0, 14, null], good.detail.values);
    });
  ...
  describe("total record", function () {
    ...
    describe("totalCredit", function () {
      testBadInputs(
        "total",
        this.title,
        ["not provided", null, undefined, "a", {}, [], 999999999],
        good.total.values
      );
    });
  ...
  describe("getEntry()", function () {
    _.each([{}, null, undefined, ""], function (v) {
      it(
        "should throw ValidationError when value is: '" + v + "'",
        function () {
          assert.throws(function () {
